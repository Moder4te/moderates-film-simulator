 ...
 *
 * ⚠️ **2026-08-17부터 기본이 `prophoto`(하이브리드)다 — 예전엔 `acr-standard`였다.**
 * `acr-standard`는 톤은 정확하지만(위 `buildXmp` 주석) **색은 안 돌린다** — 무채색
 * 벽 브래킷으로 유도해서 카메라 프로필의 색상별 변환(HueSatMap)이 안 풀린 채
 * 남는다. 채도 있는 피사체(피부·원색)에서 색이 틀어지고 계조가 깨지는 게 실사용으로
 * 확인됐다(TODO N6). 색차트로 그 부분을 재유도하기 전까지는 톤 부정확(조건 b, 이미
 * 알려진 문제)만 있는 `prophoto`(하이브리드) 쪽이 덜 나쁘다고 판단해 기본을 되돌렸다.
 * `acr-standard`는 여전히 고를 수 있다 — 무채색/저채도 사진(건물·풍경 등)에는
 * 지금도 더 정확하다.
 */
const XMP_INPUTS = [
  {
    id: "prophoto",
    displayName: "하이브리드 (ACR 커브 그대로, 기본)",
    suffix: "",
    note: "ACR 렌더링 위에 필름 커브를 그대로 얹는다 — 톤이 정확하진 않지만(조건 b) " +
      "색은 최소한 원본 ACR 렌더링 그대로다. 색차트로 acr-standard의 색 문제를 " +
      "고치기 전까지 기본값(TODO N6).",
  },
  {
    id: "acr-standard",
    displayName: "ACR 역산 (톤 정확, 색 보류)",
    suffix: " (ACR 역산)",
    note: "ACR의 숨은 톤 커브를 역산해 되돌린 뒤 필름 커브를 태운다. " +
      "Adobe Standard + 슬라이더 0 전제. ⚠️ 무채색 벽으로 유도해 색(HueSatMap)은 " +
      "안 돌아간다 — 채도 있는 피사체는 피할 것(TODO N6).",
  },
];
...
function buildXmp(params, opts) {
  ...
  // 대신 `opts.input`(위 `XMP_INPUTS`, 기본 `prophoto`)을 쓴다. 패널을
  ...
  //
  // **`acr-standard`가 이론적으로는 더 맞는 가정이다.** Lightroom도 raw를 열면
  ...
  // "Lightroom이 커브 없이 렌더링했다"고 잘못 전제하는 셈이다 — **톤만 보면 그렇다.**
  //
  // **그런데도 기본을 `prophoto`(하이브리드)로 되돌린 이유(2026-08-17, TODO N6).**
  // `acr-standard`는 무채색 벽 브래킷으로 유도해 톤만 맞고 **색은 안 돌아간다** —
  // 카메라 프로필의 색상별 변환(HueSatMap)이 그대로 남아 채도 있는 피사체(피부·원색)
  // 에서 색이 틀어지고 계조까지 깨진다. `prophoto`는 톤이 부정확한 건 여전하지만
  // (조건 b, 예전부터 알려진 문제이자 우연히 고대비 장면 다이내믹레인지를 압축해
  // 주기도 한다 — RESOLVED.md "리니어 입력 + 고대비 장면에서 채도가 무너지는 문제"),
  // **적어도 새로 생긴 색 문제는 없다.** 색차트로 `acr-standard`를 재유도하기 전까지
  // 둘 중 덜 나쁜 쪽을 기본으로 둔다. `opts.input`으로 여전히 고를 수 있다.
  //
