...
/**
 * 디더 표 길이. **소수(2^16+1 = 65537)다.**
 *
 * ⚠️ 예전에는 `1 << 16`이었고 나머지 연산을 비트 마스크로 대신했다. 그게 문제였다 —
 * 표 위치가 픽셀마다 3씩 전진하므로 폭 W인 이미지에서 한 행은 3W씩 건너뛴다.
 * **W가 2의 거듭제곱이면 65536과 인수를 공유해 몇십 행마다 표가 정확히 제자리로
 * 돌아온다.** W=2048이면 32행, W=4096이면 16행 주기로 **디더 잡음이 그대로 반복**된다.
 * 2048·4096은 내보내기 폭으로 흔하다.
 *
 * 65537은 소수라 3W와 절대 인수를 공유하지 않는다. 어떤 폭에서도 주기가 표 길이
 * 전체로 늘어나 사실상 반복이 사라진다. 대신 마스크를 못 쓰지만, 비교 한 번으로
 * 감으면 되고(`>=`면 0으로) 안쪽 루프 비용 차이는 측정되지 않는다.
 *
 * ±1LSB라 눈에 잘 안 띄는 종류였다. 그래도 무료로 없앨 수 있는 것을 남겨 둘 이유가 없다.
 */
const DITHER_N = 65537;
...
function apply(data, comps, count, opts, grading) {
  ...
  // 표 길이가 소수라 이미지 폭과 인수를 공유하지 않는다 — 위 DITHER_N 주석 참조.
  let dtab = null;
  if (useDither) {
  ...
  for (let p = 0, i = 0; p < count; p++, i += comps) {
    ...
    for (let c = 0; c < 3; c++) {
      ...
      if (useDither) {
        d = dtab[di];
        if (++di >= DITHER_N) di = 0; // 표 길이가 소수라 마스크를 못 쓴다
      }
      const q = px[c] * maxV + d;
