...
> **v2.3.0** — 필름 9종(주광 8 + 텅스텐 Vision3 500T) · 다중스케일 광학 할레이션 ·
> 값 노이즈 그레인 · 마감 그레이딩 · 피부톤 색역 · 분포 패널.
> 이번 판에 **그레인 물리 개선**(채널 독립 컬러 그레인 기본값 · 존 경계 재설계 ·
> 옥타브 회전으로 격자 인공물 완화)이 들어왔다.
...
⚠️ **Camera Raw로 현상해 그대로 먹이는 경로는 권장하지 않는다.** 실측(2026-08-12,
Sony A7RV, "Adobe Color" Look을 뗀 순정 "Adobe Standard" 프로필 + 슬라이더 전부 0)
결과 암부 +0.62 / 명부 −0.24스톱의 대비 커브가 남았다 — **이름과 달리 선형이
아니고, 오프셋 하나로 못 고치는 종류다.** 이 상태로 현상한 파일을 엔진에 먹이면
그 커브가 필름 곡선과 곱해져 룩이 통째로 달라진다. 상세 → [`docs/RESOLVED.md`](docs/RESOLVED.md)
"ACR Adobe Standard도 조건 (b)를 만족하지 않는다".
(18% 그레이 카드로 잡는 절대 앵커(조건 (a), `MID_GRAY_OFFSET`)는 아직 미확정이다 —
...
- **Camera Raw로 직접 현상하는 경로는 실측상 중립이 아니다.** "Adobe Standard" +
  ...
  실측). `tools/decode-raw.py`(raw 직접 디코드)가 공식 경로다 —
  위 [사용법](#순서가-중요하다--색-먼저-마감-나중) 참조. 18% 그레이 카드로 잡는
