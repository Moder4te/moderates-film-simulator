...
- **그레인 입자 크기**는 노이즈 생성 후 가우시안 블러로 뭉쳐 근사한다. 실제 은염
  결정 형상은 재현하지 않는다.
- **유효 입자 크기가 필름별로 다르지 않다.** 포맷과 슬라이더로만 정해진다. 실제로는
...
크기 슬라이더 0~100이 2~42µm에 대응하고, 기본값 20이 10µm(Portra 400)이다.
할레이션 반경도 같은 이유로 포맷에 따라 좁아진다.

**입자 크기 기준** — 기본은 문서 픽셀 그대로다. 작업 파일과 최종 출력 크기가
다를 때(9504px로 작업하고 2048px로 웹에 올릴 때 등) 6000 / 9000으로 고정하면
그 크기에서 맞는 입자를 만든다.

`grain_px`가 1px 미만이면 블러를 걸지 않고 노이즈 강도로 환산한다 — 픽셀보다 고운
입자는 그 자리에서 평균되어 변조 진폭이 줄어드는 것이 실제 거동이다.

> ⚠️ v1.4까지의 그레인 계산에는 물리적 근거가 없었다(긴 변 3000px 정규화 × 계수
> 2.2). **35mm 6000px에서 입자가 약 1.9배 굵어진다.** 기존 룩을 유지하려면 크기
> 슬라이더를 낮춘다.
