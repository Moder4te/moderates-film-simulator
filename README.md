...
- **무채색 미니휠**: whites/neutrals/blacks는 채도 0이라 큰 휠 중심에 뭉쳐 못 고른다.
  ...
  큰 휠에는 유채색 11개만 얹는다(`COLOR_PALETTE`). `buildMini` / `handleAchromaDrag`.
...
**재적용은 덮어쓴다**
`run`은 시작 시 이름이 `FilmSim`으로 시작하는 이전 레이어를 모두 지운다. 같은
문서에 여러 번 적용해도 결과가 쌓이지 않고 이전 내역에 덮어쓴다(검증: 3회 적용해도
FilmSim 레이어 2개 유지, 배경 보존). 단일 적용·배치 공통. — `pipeline.js`
...
- **그레인 입자 크기**는 노이즈 생성 후 가우시안 블러로 입자를 뭉쳐 근사한다.
  ...
  직접 접근해야 한다 (v2).
- **레이어 그룹화 미적용.** 결과 레이어는 `FilmSim · ` 접두사로 구분한다.
  그룹이 없으므로 "프리셋 전체 강도" 슬라이더도 아직 없다 (v2).
- **크로스토크 매트릭스** 미구현. 스펙에는 있으나 v2 범위.
