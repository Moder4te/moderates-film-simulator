...
> **v2.5.0** — Camera Raw로 그냥 현상한 파일을 위한 **ACR 역산** 입력을 추가하고
> (숨은 톤 커브를 되돌린다, 10스톱 브래킷으로 재유도해 표본 범위 확장), 코드
> 리뷰에서 나온 결함 9건을 고쳤다.
> ⚠️ **그런데 ACR 역산이 색은 안 돌린다는 게 실사용으로 드러났다** — 무채색
> 벽으로 유도해서 채도 있는 피사체(피부·원색)에서 색이 틀어진다. 색차트로
> 재유도하기 전까지 `.xmp` 내보내기 기본값을 다시 **하이브리드**로 되돌렸다.
> 자세한 내용은 [릴리스 노트](https://github.com/Moder4te/moderates-film-simulator/releases/tag/v2.5.0) 참조.
>
