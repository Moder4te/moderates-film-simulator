...
두 가지 중 하나를 고른다 — **ACR 역산(정확, 기본)**은 Camera Raw의 숨은 톤
커브를 되돌려 실제 Lightroom 렌더링 위에서 정확하게 나오고, **하이브리드
(ACR 커브 그대로)**는 그 커브를 되돌리지 않아 이론적으로는 부정확하지만 ACR
커브 자체의 다이내믹 레인지 압축이 고대비 장면에서 오히려 보기 좋게 나올 때가
있다 — 결과물이 마음에 드는 쪽을 고르면 된다. 고른 뒤 폴더를 지정하면 필름 ×
스캐너 조합을 `.xmp`로 한 번에 쓴다. 아래 폴더에 복사하고 Lightroom을
재시작하면 끝이다.
...
- **Camera Raw로 직접 현상하는 경로는 실측상 중립이 아니다.** "Adobe Standard" +
  ...
  실측). `tools/decode-raw.py`(raw 직접 디코드)가 정확도 기준 공식 경로이고,
  외부 도구 없이 쓰려면 `acr-standard` 입력(그 커브를 역산해 되돌림, 2026-08-13)이
  대안이다 — 위 [사용법](#순서가-중요하다--색-먼저-마감-나중) 참조. 18% 그레이
  카드로 잡는 절대 앵커(`MID_GRAY_OFFSET`)는 아직 미확정 — 노출 슬라이더로
  상쇄되어 급하지 않다.
- **`acr-standard` 입력은 카메라 종속이다.** Sony ILCE-7RM5 + ACR 18.3.2 한 세트로
  유도됐다 — DCP 프로필이 카메라 모델마다 달라 다른 카메라는 근사치다.
