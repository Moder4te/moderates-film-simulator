...
### decode-raw-gui.py — 데스크톱 GUI

`--probe` 좌표를 알아내려고 다른 도구로 픽셀 좌표를 찾던 단계를 없앤다. 미리보기를
클릭하면 그 자리가 곧 좌표다. **로직은 새로 만들지 않는다** — `decode-raw.py`를
...
- 파일을 여러 장 고르면 **배치**다. **미리보기(목록에서 클릭한 파일)와 기준
  프레임(측정 대상)은 분리된 별도 콤보박스다** — 미리보기는 좌표를 찍으려고
  아무 파일이나 봐도 되지만, "기준 프레임 측정"은 콤보박스로 명시적으로 고른
  파일만 잰다. 그 값을 **배치 전체에 고정 스케일로 적용**한다.
  ⚠️ **이게 핵심이다.** 노출 브래킷(같은 장면, 다른 스톱)을 프레임마다 다시 측정하면
  각 프레임이 "그 프레임의 그 자리"를 18%로 되돌리는 꼴이라 **스톱 차이 자체가
  지워진다** — 브래킷을 찍는 이유(조건 b 실측, → [`docs/RESOLVED.md`](../docs/RESOLVED.md)
  "ACR Adobe Standard도 조건 (b)를 만족하지 않는다")가 무의미해진다.
  (처음엔 미리보기 중인 파일을 그대로 쟀는데, 여러 파일을 돌아가며 값을
  확인하다 보면 — 정상적인 탐색이다 — 측정값이 마지막으로 본 파일 걸로 조용히
  덮어써지는 문제가 있었다. 지금은 콤보박스에서 명시적으로 바꿔야만 측정 대상이
  바뀌고, "변환 시작"을 누르면 기준 프레임 이름과 측정값을 확인창에 한 번 더 띄운다.)
- 서로 무관한 사진을 모아 한 번에 돌릴 때만 **"파일마다 새로 측정"**을 켠다 — 그때는
  파일마다 같은 좌표에서 각자 다시 잰다(이때는 기준 프레임 콤보박스가 안 쓰인다).
- "카드 없음"을 켜면 스케일을 안 건다(CLI에서 `--probe`·`--midgray` 둘 다 안 준 것과
