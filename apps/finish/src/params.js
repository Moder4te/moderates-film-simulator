...
function defaultParams() {
  return {
    ...
    grain: {
      ...
      feather: 40,
      // 입자 크기 0~10 → 필름면 유효 입자 지름 2~6µm. 상한 10.
      // 더 굵은 그레인은 크기가 아니라 해상도(입자 크기 기준)로 낸다 — 그레인은
      // 필름 고유 크기로 고정이고 굵어 보이는 건 확대율 문제다(format.micronsFor).
      size: 6,
      // 번짐 0~100. 해상도를 입자 크기에 묶는다. 입자보다 고운 디테일이 지워진다.
      diffusion: 85,
      // 번짐 방식. false = 블러(검증), true = 변위(실험, 미세 선을 더 잘 부순다).
      diffuseDisplace: false,
