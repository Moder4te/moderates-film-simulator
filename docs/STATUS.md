...
- ⚠️ **인화지 감마가 실측이 아니다.** `paper.js`의 `shared` 프로파일 γ 1.7066은 우리
  필름 9종 γ_G 평균의 역수다 — 평균 룩을 유지하면서 필름 간 대비 차이를 되살리려고
  잡은 값이지, RA-4 인화지에서 잰 값이 아니다(실제 RA-4는 γ ≈ 2.5~3.0).
  **기본값은 여전히 `normalized`(구 동작)이라 룩은 안 바뀐 상태다.**
  → [`TODO.md`](../TODO.md) S3
