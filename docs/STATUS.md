...
| 엔진 | engine **2.8.0** | `apps/engine/manifest.json` |
| 마감 | finish **2.15.0** | `apps/finish/manifest.json` |
...
| 피부톤 색역 | ✅ | 색상각+채도 창. 피부 보호·스포이드 ⚠️ 전경색 읽기 미검증 |
...
- **피부 스포이드**(engine v2.8) — `app.foregroundColor` 읽기가 실기에서 되는지.
  실패하면 상태줄에 알리고 대역 중심은 그대로 둔다(기본 19.5°로도 동작)
- **할레이션 축소 피라미드**(v2.15) — `transform` 디스크립터가 실기 미검증이다.
  실패해도 폴백으로 느려질 뿐이지만, **할레이션 모양이 이전과 같은지** 눈으로 확인할 것
