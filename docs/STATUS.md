...
- ⚠️ **베이스(「중립 현상」)가 검증된 적이 없다.** 정의와 검사 절차는
  [`ARCHITECTURE.md`](./ARCHITECTURE.md) 「중립 현상이 정확히 무엇인가」에 적었지만,
  **그 절차를 실제로 돌린 적이 없다.** Camera Raw는 슬라이더를 0으로 둬도 선형이
  아니라(카메라 프로필의 베이스라인 톤 커브) 지금 색감이 어떤 톤 위에 얹혀 있는지
  아무도 모른다. 색공간 어긋남은 이제 적용 시 경고하지만, **현상 곡선이 휘었는지는
  픽셀만 봐서 알 수 없다.**
  두 조건의 무게가 다르다 — **앵커 어긋남은 노출 슬라이더와 대수적으로 동일해
  상쇄되지만**(실측 1.28e-13/255), **곡선 휨은 어떤 손잡이로도 못 고친다**
  (대비 1.10배 → 암부 −3.3 · 하이라이트 +4.3). 그래서 급한 쪽은 선형성이고,
  **그건 그레이 카드 없이 벽 하나 브래킷으로 잴 수 있다.**
  → [`TODO.md`](../TODO.md) N1
- **중간 그레이 앵커 오프셋이 0으로 박혀 있다.** 기준 스캔이 없어 추정값을 넣지 않았다.
  **위 항목과 같은 문제다** — "기준 스캔"이 곧 그 브래킷 5장이다.
