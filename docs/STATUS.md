...
- **마감 그레이딩**(v2.14) — 적용·속도는 실기 확인됨. 아직 안 본 것:
  8bit JPEG에서 띠가 실제로 안 보이는지 · 슬라이더 감도가 손에 맞는지

## 성능 (실기 체감)

| 단계 | 체감 | 비고 |
|---|---|---|
| 그레이딩 + 그레인 | 합쳐서 **5초 이내** | 24MP 기준. tone은 LUT화로 4.4초 → 0.9초 |
| **할레이션** | **느리다** | 아래 참조. 따로 적용하는 것으로 우회 중 |

**할레이션이 느린 이유** — 스케일 3개를 각각 **풀해상도**에서 처리한다. 스케일마다
레이어를 복제하고 채널별로 Maximum(원반) + 가우시안을 거는데, Bleed는 반경 배율이
×4라 6000px 문서에서 수백 px가 된다. 큰 반경 필터는 면적에 비례해 비싸다.
→ 개선안은 [`TODO.md`](../TODO.md) F5.
