...
| 노광 보정 | ✅ | ±2스톱 |
| 입력 소스 — ACR 역산 | ⛔ **사용 보류, 더 이상 기본 아님** | `acr-standard`. 톤 커브는 표본 v=0.0196~1.0까지 실측(2026-08-16 재유도, 명부는 코드 최댓값까지). **그런데 무채색 벽으로 유도해서 색 변환(HueSatMap)은 안 풀렸다** — 채도 있는 피사체에서 색이 틀어지고 계조가 깨진다(2026-08-17 실사용 보고). `.xmp` 내보내기 기본값을 `prophoto`(하이브리드)로 되돌렸다 — 색차트로 재유도하기 전까지 채도 있는 사진엔 하이브리드 또는 `decode-raw.py`(`linear-h*`)를 쓸 것 → TODO N6 |
...
- ✅ **인화지는 실측이 하나 들어왔다** — Kodak ENDURA Premier(E-4070 p4, RA-4,
  ...
  → [`TODO.md`](../TODO.md) S3
- ⚠️ **`acr-standard`는 표본 아래쪽만 외삽이다.** 2026-08-16 10스톱 재유도로 표본이
  v 0.0196~1.0으로 넓어졌다 — 명부는 코드 최댓값(v=1)까지 실측이라 더 이상
  외삽이 없고, v<0.0196(깊은 그림자, 물리적 하한)만 `ln v` 축 끝점 기울기로
  이었다. → [`docs/RESOLVED.md`](./RESOLVED.md) "ACR 역산 표본을 10스톱
  브래킷으로 재유도했다"
- ⛔ **`acr-standard`는 색을 안 되돌린다 — 채도 있는 피사체엔 쓰지 말 것.**
  무채색 벽으로 유도해 톤 커브(밝기)만 맞고, 카메라 프로필의 색상별 색 변환은
  그대로 남아 있다. 피부·원색에서 색이 틀어지고 계조도 깨진다. → [`TODO.md`](../TODO.md) N6
- ⚠️ **닷지·번은 완전히 단조가 아니다.** 극단 발끝(v 0.05~0.16)에서 누적 하강이
  최대 2.0/255 남는다 — 압축 이득 하한(0.5)으로 11.74에서 줄인 잔여이고, 완전히
  없애려면 채도를 포기해야 한다. → `core/color/film.js` `DB_GAIN_FLOOR`,
  [`RESOLVED.md`](./RESOLVED.md) "닷지·번이 그레이축을 역전시켰다"
- ⚠️ **리니어 입력 × 직선 인화지는 지원 조합이 아니다.** 어깨 없는 인화지가 담을 수
  있는 것은 기준 위 약 2.5스톱뿐인데 리니어 입력은 +4~+6스톱을 싣는다. 적용 시
  경고가 뜬다(`inputs.combinationWarning`). 실측 곡선 인화지를 쓸 것
