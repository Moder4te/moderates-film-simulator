...
| 노광 보정 | ✅ | ±2스톱 |
| 입력 소스 — ACR 역산 | ⚠️ **실험적, 계조 버그 있음** | `acr-standard`. Camera Raw로 그냥 현상한 파일을 외부 도구 없이 먹인다. Sony A7RV 1대에서 유도 — 다른 카메라는 근사치. **표본 범위(v=0.0667~0.9647) 밖은 평평하게 고정돼 실사진의 깊은 그림자·거의 흰 하이라이트에서 계조가 깨진다** → TODO N4 |
| 닷지·번 | ✅ **실험적** | 인화 직전 luma 압축+대비 복원. 기본 꺼짐 — 리니어/ACR역산 입력의 고대비 장면용. 기본값 실기 확인 전 |
...
색을 계산하지 않는다. 엔진이 구운 `.xmp` 36종(필름 9 × 스캐너 2 × 입력 가정 2 —
ACR 역산/하이브리드)을 담아 설치·적용만 한다. 하이브리드 변형은 필름 이름에
접미사를 붙여 별개 항목으로 노출한다(`Apply.lua` 코드 변경 없이).
...
- **베이스(「중립 현상」) 검증 완료 (2026-08-12) — ACR 경로는 조건 (b)를 만족하지
  못한다.** 브래킷 5장(입사식 노출계 EV8, Adobe Standard, 슬라이더 0)을 픽셀 단위로
  실측한 결과, 암부 +0.62스톱 / 명부 −0.24스톱의 매끈한 S자 대비 커브가 나왔다
  (허용 오차 0.05스톱을 크게 벗어남). "Adobe Standard"라는 이름과 달리 DCP
  프로필 또는 ACR 렌더링 자체에 베이스라인 대비가 박혀 있다 — **곡선 휨이라 어떤
  손잡이로도 못 고친다.** `MID_GRAY_OFFSET`을 이 데이터로 정하지 않았다(정하면
  캘리브레이션한 노출에서만 맞고 나머지가 더 틀어진다). **결론: `tools/decode-raw.py`
  (리니어 현상)가 이 프로젝트의 공식 중립 현상 경로다** — 조건 (b)를 구성상
  만족시킨다. → [`ARCHITECTURE.md`](./ARCHITECTURE.md) 「검증됨 — ACR 경로는 조건
  (b)를 만족하지 못한다」, [`RESOLVED.md`](./RESOLVED.md)
- **중간 그레이 앵커(`MID_GRAY_OFFSET`)는 여전히 0.** 위 결론에 따라 ACR 경로 자체를
  쓰지 않기로 했으므로 이 상수로 잴 이유가 없어졌다. 그레이 카드로 (a)를 잡는 일은
  급하지 않고(노출 슬라이더로 상쇄됨), 필요해지면 `decode-raw.py --probe`로 한다.
