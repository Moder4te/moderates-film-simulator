...
순서대로 읽을 필요는 없지만, 아래는 먼저 알고 시작하는 편이 낫다.
...
| 산출 레이어가 엉뚱한 그룹 안에 생긴다 | `make layer`는 활성 레이어 옆에 놓는다 — 그룹이 활성이면 그 **안** | 2.3 |
...
### 2.3 새 레이어는 **활성 레이어 옆**에 생긴다 — 그룹을 고르면 그 안이다

`make layer`는 위치를 지정하지 않으면 활성 레이어를 기준으로 놓인다. 그래서 산출물의
위치가 **사용자가 무엇을 선택해 뒀는지에 따라 달라진다.**

특히 **활성이 그룹이면 새 레이어가 그룹 위가 아니라 안에 생긴다.** 레이어 패널에서
접힌 그룹을 고르고 새 레이어 버튼을 누르는 것과 **다르다** — 그 UI 동작을 그대로
가정했다가 틀렸다.

실측(2026-07-29):

```
① doc.layers 순서 : 0:그룹 2[그룹8]  |  1:배경     ← 위 → 아래 순
② layers[0] = 그룹 2 (그룹)
③ 만든 레이어 : 레이어 1 | 최상위인가 : false
④ 만든 뒤 순서 : 0:그룹 2[그룹9]  |  1:배경        ← 자식이 8 → 9
⑤ parent : 그룹 2
```

두 가지가 함께 확인됐다.

- **`doc.layers`는 위→아래 순서다.** `[0]`이 최상단
- **그룹을 앵커로 쓰면 안 된다.** 산출물이 그 안에 갇힌다

이게 실제로 낸 결함 — 재적용 시 `clearOwnLayers`가 이전 그룹을 지우면 선택이 남은
그룹으로 떨어지고, 그 상태로 만들어 마감 레이어 8장이 전부 사용자 그룹 안에 생겼다.
`groupOwnLayers`는 최상위만 보므로 못 찾아 **`FilmSim Finish` 그룹이 아예 안 만들어졌다.**

⚠️ 자리만의 문제가 아니다 — 산출물이 그룹 안에 들어가면 그 그룹의 블렌드·불투명도·
마스크가 결과에 **다시** 곱해진다. **다만 이건 구조상 그렇다는 것이고 실기로 재 보지
않았다**(사고가 난 문서의 그룹은 Normal 100%였다).

**실측으로 확인된 것은 따로 있다** — 쌓임 순서가 뒤집혔을 때 깨지는 것은 "가려짐"이지
"색"이 아니다. `getPixels`가 문서 전체 합성본을 읽으므로 산출물이 어디 놓이든 구워지는
내용은 같다.

→ `host/ps.js`의 `anchorTopLevel` — 만들기 전에 **그룹이 아닌 최상위 레이어**를 골라
삽입 지점을 고정한다.
