...
#### 두 조건의 무게가 다르다 — (a)는 노출과 같고, (b)는 못 고친다

먼저 이걸 알고 절차를 봐야 한다. 실측(Portra 400 + Endura, 격자 없이 응답 함수 직접):

**(a) 앵커 오차는 노출 시프트와 대수적으로 동일하다.** `H = log10(L/0.18) + ev·log10(2)`
이므로 "L에 상수를 곱하는 것"과 "ev를 더하는 것"이 같은 연산이다. 앵커가 +0.4스톱
어긋난 것을 노출 −0.4스톱으로 되돌리면 **최대차 1.28e-13/255** — 완전히 같다.
즉 **눈으로 노출을 맞추면 (a)는 상쇄된다.**

남는 비용은 셋뿐이고 전부 부수적이다.
  · 숨은 상수 오프셋을 모른 채 매번 보정하게 된다
  · `.cube`·`.xmp`를 내보내면 그 오프셋이 따라간다(남의 환경에서 틀어진다)
  · **하이라이트 여유가 준다** — 앵커가 +0.4스톱 높으면 선형 0.758 위가 이미 잘린다.
    이건 노출로 못 되돌린다

**(b) 곡선 왜곡은 어떤 손잡이로도 못 고친다.** 입력에 대비 1.10배짜리 휨이 걸린 상태를
노출로 **최선 보정**한 뒤 남는 잔차:

| 톤 | 정상 | 휜 입력 | 차 |
|---|---|---|---|
| 암부 −3스톱 | 25.4 | 22.1 | **−3.3** |
| −1스톱 | 65.3 | 61.0 | −4.4 |
| 기준 18% | 98.4 | 96.6 | −1.8 |
| +1스톱 | 134.9 | 136.7 | +1.8 |
| +2스톱 | 167.6 | 171.9 | **+4.3** |

오프셋이 아니라 **회전**이라 단일 손잡이로 안 펴진다(1.20배면 잔차 11.3/255).
그리고 이게 우리가 하려는 일과 정면으로 충돌한다 — 필름의 발끝·어깨를 인화지 곡선으로
만들어 뒀는데 입력에 이미 S커브가 있으면 **두 곡선이 곱해진다.**

#### 검사 절차 — 브래킷이 본질, 그레이 카드는 선택

위 때문에 **(b)가 진짜 검사**다. (a)만 재면 S커브가 걸려 있어도 통과한다.

**(b) 선형성 — 기준 타깃이 필요 없다.**

1. **정적인 장면 아무거나** 노출만 `-2 / -1 / 0 / +1 / +2`스톱으로 바꿔 5장 찍는다
   (벽·하늘이면 충분하다. 조명·조리개·포커스는 고정).
2. 검사할 현상 설정으로 현상해 **ProPhoto 16bit**로 내보낸다.
3. **같은 자리의 패치 하나**를 골라 인코딩값 `v`를 읽는다(0~255면 ÷255).
   ⚠️ 클리핑되지 않는 중간 밝기를 고를 것.
4. `log2(v^1.8)`가 **정확히 1.0씩** 움직여야 한다. 편차 0.05스톱을 넘으면 톤 커브가
   걸려 있는 것이고, **오프셋으로 못 고친다 — 현상 설정을 바꿔야 한다.**

**(a) 앵커 — 여기에만 18% 그레이 카드가 필요하다.**

5. 0스톱 장의 그레이 패치가 **0.386 ± 0.005**(8bit 98±1)인가.
6. 어긋나면 `film.js`의 `MID_GRAY_OFFSET`으로 **한 번만** 맞춘다. 전역 상수다 —
   필름마다 다른 값을 주면 감도 차이를 이중 적용하게 된다.

카드가 없으면 5·6은 건너뛰어도 된다. **(b)만 통과시켜도 룩의 형태는 맞고**, 남는 것은
노출 슬라이더로 상쇄되는 상수와 하이라이트 여유뿐이다.

#### 검증됨 — ACR 경로는 조건 (b)를 만족하지 못한다 (2026-08-12 실측)

브래킷 5장(±2/±1/0스톱, 입사식 노출계 EV8 기준, ACR "Adobe Standard" 프로필 · Look 없음 ·
슬라이더 전부 0 · ProPhoto 16bit)으로 재봤다. 같은 물리적 지점을 프레임 간 픽셀 단위로
직접 비교(1.8억 화소, 클리핑 화소 제외) — **조건 (b)가 크게 어긋난다.**

| 밝기 구간(어두운 쪽) | 실측 Δstop | 편차 |
|---|---|---|
| 암부 (v0 0.14~0.34) | +1.62 | **+0.62** |
| 중간 | +1.01~+1.13 | 0~+0.13 |
| 명부 (v0 0.53~0.82) | +0.76 | **−0.24** |

허용 오차(0.05스톱)를 크게 벗어나고, 밝기가 오를수록 게인이 줄어드는 **매끈한 S자
대비 커브** 형태다 — "Adobe Standard"라는 이름과 달리 DCP 프로필 또는 ACR 렌더링
자체에 베이스라인 대비가 박혀 있다는 뜻이다. **오프셋(회전이 아니라 상수)으로는
못 고치는 종류**라 `MID_GRAY_OFFSET`을 이 데이터로 정하지 않았다 — 정하면 캘리브레이션한
그 노출에서만 맞고 나머지는 오히려 더 틀어진다.

**결론: ACR 출력을 보정 없이 그대로 중립 현상으로 쓰지 않는다.** 두 가지 대안이
있다 — 아래 「리니어 현상」(`tools/decode-raw.py`)은 조건 (b)를 **구성상** 만족시키고
(ACR을 아예 안 거친다), 다음 절의 「ACR 역산」(`acr-standard` 입력)은 이 커브를
**측정해서 되돌린다**(ACR을 거치되 보정한다). `MID_GRAY_OFFSET`은 여전히 0으로
둔다 — "ACR 경로 자체가 이 상수로 못 고치는 곡선을 갖고 있어서"다. 카드로 (a)
앵커까지 잡는 일은 급하지 않다(노출 슬라이더로 상쇄됨).
전체 실측치·현상 설정은 → [`RESOLVED.md`](./RESOLVED.md) "ACR Adobe Standard도
조건 (b)를 만족하지 않는다"

#### 조건 (b)를 **측정해서 되돌리는** 길 — ACR 역산 (2026-08-13)

decode-raw.py는 ACR을 아예 안 거치는 대신 색 렌더링·렌즈 보정 같은 ACR의 편의도
같이 잃는다. `acr-standard` 입력은 반대로 간다 — Camera Raw로 "Adobe Standard" +
슬라이더 0으로 그냥 현상하고, **그 위에서 커브를 되돌린다.**

방법은 위 실측과 같은 브래킷을 다시 쓴다. HDR 카메라 응답함수 복원(Debevec–Malik
1997)과 동형 문제다 — 여러 물리적 지점에서 정확히 아는 상대 노출(ISO로 정확히
1스톱씩)과 관측된 렌더링값을 최소자승에 넣어 "렌더링값 → 실제 상대 로그노광"
응답함수를 복원한다(`tools/derive-acr-curve.py`).

**왕복 검산** — 복원한 곡선으로 같은 브래킷의 스톱 간격을 다시 재면 0.61~1.58로
요동치던 것이 0.94~1.02로 모인다. **교차검증** — 고대비 실사진(MDR03671)에
`acr-standard`와 `linear-h5`(decode-raw.py, 완전히 다른 경로) 양쪽을 걸어 보니
스톱 위치가 서로 수렴했다 — 방법이 다른 두 보정이 같은 "진짜 노광" 자리로
모인다는 것이 상호 증명이다.

⚠️ **카메라·프로필 종속이다.** Sony ILCE-7RM5 + ACR 18.3.2 한 세트에서 유도됐다 —
DCP 프로필이 카메라마다 달라 다른 카메라는 근사치다.
⚠️ **명암 넓은 장면은 이것만으로 부족하다.** `acr-standard`는 입력을 정확하게
고치는 것이지, 필름+인화지의 편한 구간(±1.5스톱)보다 넓은 장면을 다루는 건 별개
문제다 — 그건 인화 단계의 **닷지·번**(`core/color/film.js`의 `opts.dodgeBurn`,
`buildLut` 참조)이 맡는다. 둘은 상호보완이지 대체가 아니다. 상세 →
[`RESOLVED.md`](./RESOLVED.md) "ACR 'Adobe Standard' 톤 커브를 역산해 되돌리는
입력 추가" · "리니어 입력 + 고대비 장면에서 채도가 무너지는 문제"

#### ⛔ 조건 (b)만으로는 부족하다 — 색은 별개 조건 (c)다 (2026-08-17, TODO N6)

위 두 조건(a·b)은 처음부터 **R·G·B가 전부 같은 스칼라 곡선을 탄다**는 것을
전제한다 — "(b) 전달함수가 감마 1.8 거듭제곱 **하나뿐**"이라는 표현 자체가 그
뜻이다. `tools/derive-acr-curve.py`도 R·G·B 표본을 한 데이터셋으로 합쳐 채널
공통 곡선 하나만 푼다. 그런데 실제 카메라 프로필(DCP)은 톤 커브 말고도
**HueSatMap**(색상·채도별로 다르게 트는 3D 룩업)을 포함한다 — 채널 공통 스칼라
곡선으로는 원리적으로 못 되돌리는 변환이다.

**실사용으로 드러났다.** N1braket이 무채색 벽이라 이 변환 자체가 표본에 안
잡혔고, 그래서 왕복 검산·저채도 노출 정합은 통과했다. 그런데 피부·원색처럼
채도 있는 피사체에서는: ① `prophoto` 입력에서는 안 보이던 계조 깨짐이
`acr-standard`에서만 보이고, ② 입력 소스만 바꾸고 나머지를 맞춘
`linear-h5`(decode-raw.py) 사진과 색이 완전히 달라진다. **`acr-standard`는
지금 무채색/저채도 피사체에서만 신뢰할 수 있다** — 채도 있는 실사진에는
`decode-raw.py`(`linear-h*`)를 쓸 것. 상세·해결 방향 → [`TODO.md`](../TODO.md) N6

#### 조건 (b)를 **측정 대신 구성으로** 만족시키는 길 — 리니어 현상

ACR을 믿고 재는 대신, raw를 톤 커브 없이 직접 풀면 (b)가 구성상 성립한다.
`tools/decode-raw.py`가 libraw로 그렇게 뽑는다(`gamma=(1,1)`, `output_color=ProPhoto`,
`no_auto_bright=True`).

그러면 문제가 하나 생긴다 — **0~1 통에 씬을 어디 놓을 것인가.** 기준 그레이를 0.18에
두면 위로 **2.47스톱**뿐이라 하늘·창·스페큘러가 잘린다. 지우려는 그 베이스라인 톤
커브가 사고 있던 것이 정확히 그 범위다.

그래서 기준 그레이를 내려 담고, 엔진이 그만큼 되올려 읽는다
(`core/color/inputs.js`의 `linear-h{N}`, `decode(v) = v^1.8 · 0.18 · 2^N`).

| 헤드룸 | 기준 그레이 인코딩 | 8bit | 근거 |
|---|---|---|---|
| +4 | 0.2143 | 55 | 측정된 C-41 어깨(4.0~4.5스톱)를 겨우 품는다 |
| **+5 (기본)** | **0.1458** | **37** | **경계** — 위로는 스톱당 3코드값 미만 |
| +6 | 0.0992 | 25 | 여유는 남지만 얻는 것이 없다 |

**+5가 경계인 근거**(전 필름 × Endura, 기준 위 스톱당 출력 증가, 8bit):

| | +1 | +2 | +3 | +4 | +5 | +6 |
|---|---|---|---|---|---|---|
| Portra 400 (C-41) | 36.5 | 32.7 | 25.3 | 15.1 | 9.0 | 3.5 |
| Vision3 500T (ECN-2) | 35.8 | 32.4 | 25.5 | 15.1 | 8.9 | 3.6 |
| Agfa Ultra 50 (최고대비 C-41) | 56.3 | 39.1 | 17.5 | 7.6 | 2.1 | 0.9 |

+5 위로는 모델이 스톱당 3코드값 미만으로만 움직여 통이 잘라도 필름 롤오프와 구분되지
않는다. +4에서 자르면 아직 살아 있는 계조(7.6~15.1)를 **디지털로** 자르게 된다.

> ⚠️ **ECN-2가 더 관용적이지 않다** — 적어도 우리 데이터에서는. Vision3와 Portra 400의
> 스톱별 응답이 소수점까지 사실상 같다. 관용도를 가르는 것은 공정이 아니라 **대비**다.
> 실측된 어깨는 Agfa 두 종(4.0 / 4.5스톱)뿐이고 나머지 7종은 TDS가 직선부에서 잘렸다.

⚠️ **도구와 패널의 헤드룸이 같아야 한다.** `-h 5`로 뽑았으면 패널도 「리니어 +5스톱」.
어긋나면 통째로 노광이 밀린다. 도구가 끝에 무엇을 고르라고 찍어 준다.
⚠️ **눈으로 보면 어둡다**(기준 그레이 8bit 37). 보는 파일이 아니라 먹이는 파일이고,
그래서 16bit가 필수다. Photoshop에서 **프로파일 지정 → ProPhoto RGB**도 해야 한다.

#### 예외 — 로그 소스

입력 전달함수는 `core/color/inputs.js`가 소유하고, 패널(`params.film.input`)과
내보내기(`cube.build`의 `input` 옵션) 양쪽에서 고른다 — **엔진과 내보내기가 같은
정의를 써야** 네 경로가 갈라지지 않는다.
Sony S-Log3을 고르면 위 (a)(b) 대신 **S-Log3 규약**이 기준이 된다 — 18% 그레이가
코드 `420/1023 = 0.41056`, 코드 1.0이 선형 38.4(+7.74스톱)다.

**S-Log3만 내보내기 전용이다** — 원색 변환(S-Gamut3.Cine → ProPhoto 3×3)이 필요한데
`film.js`는 채널 독립을 전제로 축별 1차원 표를 쓴다. 리니어 현상본은 톤 축만 다르므로
적용 경로에서도 쓴다. 어느 쪽이든 기본값이면 `Math.pow(v, 1.8)` 그대로라 **비트 단위로
같다**(정합성 검사가 확인한다).
