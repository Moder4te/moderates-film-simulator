...
(ISO 측광 이론의 `8.1/S`와 TDS의 `14.4/S`가 0.25 log 어긋난다)는 **(a)와 같은
문제**다 — "정답 스캔이 없다"의 정답 스캔이 곧 위 브래킷이다.
**(b)는 그보다 급하다.** 카드가 없어도 지금 바로 잴 수 있고, 어긋나 있으면 지금까지의
모든 룩 판단이 정체불명의 커브 위에서 이뤄진 것이 된다.
→ [`TODO.md`](../TODO.md) N1
