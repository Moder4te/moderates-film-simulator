...
**결론: ACR을 이 프로젝트의 중립 현상 경로로 쓰지 않는다.** 아래 「리니어 현상」
(`tools/decode-raw.py`)이 조건 (b)를 구성상 만족시키는 **공식 경로**다. `MID_GRAY_OFFSET`은
여전히 0으로 둔다 — 이제는 "안 재봐서"가 아니라 "ACR 경로 자체가 이 상수로 못 고치는
곡선을 갖고 있어서"다. 카드로 (a) 앵커까지 잡는 일은 급하지 않다(노출 슬라이더로
상쇄됨) — 하려면 `decode-raw.py --probe`로 한다.
